...
# --- Dépendances ---
command -v jq &>/dev/null || fail "jq est requis mais introuvable  →  brew install jq  /  apt install jq"
ok "jq disponible"
...
if jq -e '.statusLine' "$SETTINGS" > /dev/null 2>&1; then
  warn "La clé statusLine existe déjà dans settings.json — ignoré (modifiez-la manuellement si besoin)"
else
  PATCH='{
    "statusLine": {
      "type": "command",
  ...
  ok "statusLine ajouté dans $SETTINGS"
fi
...
echo "Relancez Claude Code pour activer la status line."
echo ""
